...
The premise of the game is simple: eliminate all opposing players to win. Which you can do by taking out all their pieces & denying them points.

One of the main ideas behind this was to make it a turn-based game, but everyone makes their moves simultaneously, making strategy more necessary.
...
The game's 'Board' consists of an infinitely sized array of hexagons, opposed to square tiles. Which allows pieces to have a wider array of possible moves.
...
The player desiring to host a match would load the server webpage, which facilitates all online connectivity, players who want to participate can load the standard game webpage and then enter the game session code generated by the server webpage.
...
Because of the lack of official developer mod support, all mods must be powered by community developed tools.
...
The first tool in the collection was our heavily Visual Studio inspired Tag file viewer (Tag files being the files to store game asset’s data).

The purpose of this tool is to view and edit the game asset’s container files (module files), which is the fundamental feature of being able to make/modify content for the game engine.
...
The marching cubes algorithm itself is a simple set of rules that dictate what type of shape should be generated, given 8 vertices on a cube.
Each vertex holds a single bit of data: whether it is inside or outside of the shape.
By knowing which of the 8 vertices are inside the shape, we can calculate the mesh segment inside that cube, for a total possible of 256 shapes. With this algorithm, we can simply calculate the mesh from as many adjacent cubes as we need, hence ‘marching’.
...
by defining how solid each vertex is, we can calculate the exact contour position between each pair of vertices, allowing us to generate smoother meshes (or rather, a much larger array of meshes).
...
Which simply put, is an issue where two adjacent cubes might share similar shapes but both shapes assume the other has a triangle covering the hole in between.
...
Next up was investigating opcodes that did either did nothing or didn't work as expected, and as it would turn out some of them were intentionally disabled by the developers.
...
The goal of these projects were to bring entirely new & unique experiences to all of the players of Halo MCC.
...
Which was entirely unexpected, else the project would have ended there as an example project (like so many others I had worked on).
...
'Invasion' being an asymmetric player vs player game mode, where teams of players would take turns at attacking and defending numerous objectives, featuring probably the most immersive Halo multiplayer experience. 
...
Following, millions of lines of code would be committed the project, and somewhere around thousands of users downloading and using the tool.
...
As of writing there is still no tool which supersedes or replaces the functionality of this tool.
